#!/usr/bin/env python3
"""
Hecatech AI Agent — MOCK Runner (không cần API key)
Render output từ mock/sample data có sẵn, giả lập đúng format Lark của cả 2 agent.

Usage:
    python3 run_mock.py               # chạy cả 2 agent
    python3 run_mock.py project       # chỉ Agent #1 (per-project XKMVN)
    python3 run_mock.py ceo           # chỉ Agent #5 (CEO Briefing)
"""
...
# ──────────────────────────────────────────────
# MOCK OUTPUT — Agent #1 Per-Project (XKMVN)
# ──────────────────────────────────────────────
MOCK_PROJECT_OUTPUT = {
    "project": "XKMVN",
    "date": "2026-04-12",
...
# ──────────────────────────────────────────────
# MOCK OUTPUT — Agent #5 CEO Briefing
# ──────────────────────────────────────────────
MOCK_CEO_OUTPUT = {
    "date": "2026-04-12",
    "headline": {
...
# ──────────────────────────────────────────────
# RENDER FUNCTIONS (copy từ test_local.py + test_ceo_briefing.py)
# ──────────────────────────────────────────────
...
def run_project():
    print("\n" + "█"*60)
    print("  AGENT #1 — PER-PROJECT WARNING (MOCK)")
    print("  Dữ liệu: sample_data.json — XKMVN — 2026-04-12")
    print("█"*60)
    print(render_project_card(MOCK_PROJECT_OUTPUT))
    # Lưu JSON
    out = SCRIPT_DIR / "mock_output_project.json"
    out.write_text(json.dumps(MOCK_PROJECT_OUTPUT, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"💾 JSON đã lưu: {out.name}")


def run_ceo():
    print("\n" + "█"*65)
    print("  AGENT #5 — CEO BRIEFING (MOCK)")
    print("  Dữ liệu: sample_all_projects_outputs.json — 11 dự án — 2026-04-12")
    print("█"*65)
    print(render_ceo_briefing(MOCK_CEO_OUTPUT))
    out = SCRIPT_DIR / "mock_output_ceo.json"
    out.write_text(json.dumps(MOCK_CEO_OUTPUT, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"💾 JSON đã lưu: {out.name}")
...
def main():
    ...
    print("\n🚀 HECATECH AI AGENT — MOCK MODE (không cần API key)")
    print("   Output giả lập từ sample data — format giống hệt production Lark")
    ...
    if mode not in ("all", "project", "ceo"):
        print(f"❌ Unknown mode: {mode}. Dùng: project | ceo | (bỏ trống = cả 2)")
        sys.exit(1)

    print("\n✅ Xong! Nếu output OK → set ANTHROPIC_API_KEY rồi chạy thật:")
    print("   export ANTHROPIC_API_KEY=\"sk-ant-xxxxx\"")
    print("   python3 test_local.py sample_data.json")
    print("   python3 test_ceo_briefing.py sample_all_projects_outputs.json\n")
